...
ComedianBookingMenu::ComedianBookingMenu(HINSTANCE* currentInstance, const char* name, WNDPROC window) : DefaultWindowClass(currentInstance, name, window)
{
...
LRESULT CALLBACK ComedianBookingMenu::WndProcedure(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	if (first_run == false)
...
BOOL ComedianBookingMenu::createComedianBooking(HWND hWnd)
{
	name = CreateWindow(
		TEXT("static"),
		TEXT("Name"),
...
void ComedianBookingMenu::saveBookedComedian(TCHAR* eventDetails, int size)
{
	first_run = true;
...
void ComedianBookingMenu::createInitialWindow(HWND hWnd)
{
	first_run = true;
